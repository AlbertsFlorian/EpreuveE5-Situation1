...
class EmployeController extends AbstractController
{
    ...
    public function new(Request $request, EmployeRepository $employeRepository): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $pass = md5($utilisateur->getMdp());// hashage du mot de passe avec md5
            $utilisateur->setMdp($pass);//mot de passe crypté
            $utilisateur->setStatut(1);//par défaut un nouvel utilisateur aura un role classique
            $entityManager->persist($utilisateur);
            $entityManager->flush();

            return $this->redirectToRoute('employe_index');//on ne veut pas que l'utilisateur ait accès à la liste de tous les autres utilisateurs
        }
